...
class MCTS_Task(SearchTask):
    def __init__(self, data, propose_method='glm', value_method='glm', branch=3, end_gate=0.9, roll_policy='greedy',
                 ...
                 evaluate='', sample_value='simple', answer=None, verify_method='string', lang='zh', weighted_verify=False, client=None, VALUE_BASE_MODEL_DIR='', served_model_name=None):
        ...
        self.client = client
        self.VALUE_BASE_MODEL_DIR = VALUE_BASE_MODEL_DIR
    ...
    def get_next_step(self, y, step_n):
        ...
        response = get_proposal(prompt, self.propose_method, self.temperature, self.max_tokens, self.seed,
                                self.max_length,
                                self.truncation, self.do_sample, self.max_new_tokens, client=self.client, served_model_name=self.served_model_name)
    ...
    def get_simple_reflection(self, y, step_n):
        ...
        while not response and cnt:
            response = get_proposal(reflection_prompt, self.propose_method, self.temperature, self.max_tokens,
                                    self.seed,
                                    self.max_length,
                                    self.truncation, self.do_sample, 128, client=self.client, served_model_name=self.served_model_name)
            cnt -= 1
        if not response:
            print('获得意见失败！\n')
            return '<end>'
    ...
    def get_step_value(self, y):
        ...
        if self.value_method == 'local':
            ...
            value = get_value(prompt_answer, self.value_method, self.temperature, self.max_tokens, self.seed,
                              self.max_length, self.low, self.high, VALUE_BASE_MODEL_DIR=self.VALUE_BASE_MODEL_DIR, client=self.client, served_model_name=self.served_model_name)
            print(f'获得评分:{value}\n')
    ...
    def get_MATH_summary(self, y):
        ...
        response = get_proposal(prompt, self.propose_method, self.temperature, self.max_tokens, self.seed,
                                self.max_length,
                                self.truncation, self.do_sample, 256, client=self.client, served_model_name=self.served_model_name) #有些是在做继续推理，而没有输出 the final answer:
        if not response:
            print('获得综述失败！\n')
            return ''
        p = ''
    ...
    def get_MATH_critic(self, y):
        ...
        response = get_proposal(prompt, self.propose_method, self.temperature, self.max_tokens, self.seed,
                                self.max_length,
                                self.truncation, self.do_sample, 1024, client=self.client, served_model_name=self.served_model_name)
        if not response:
            print('获得反思失败！\n')
            return ''
        p = ''
