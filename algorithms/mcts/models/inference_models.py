...
import base64
def get_local_response_api(query, tokenizer=None, max_length=2048, truncation=True, max_new_tokens=2048, temperature=0.7, do_sample=False, pipe=None, processor=None, client=None, served_model_name=None):
    cnt = 2
...
VALUE_PROMPT = """
You are an expert reasoning evaluator. Given a **question** and a **solution**, estimate the probability (between 0 and 1) that this solution can be considered a correct and complete final answer to the question.

Carefully consider the relevance, correctness, completeness, and logical coherence of the solution with respect to the question. The score should be:

- Close to **1.0** if the solution clearly and correctly answers the question with coherent reasoning.
- Around **0.5** if the solution is partially correct or incomplete.
- Close to **0.0** if the solution is incorrect, irrelevant, or misleading.

Format your answer as a **single float between 0 and 1**, Do not output anything else.
"""
def get_local_value_api(query, tokenizer=None, max_length=2048, truncation=True, max_new_tokens=2048, temperature=0.7, do_sample=False, pipe=None, processor=None, client=None, low=0, high=1, served_model_name=None):
    cnt = 2
    all_response = ''
        ...
        try:
            chat_response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": "Input: {query}\nOutput: <Probability Score (0-1)>".format(query=query),
                    },
                ],
            )
            v_response = chat_response.choices[0].message.content
            # print('** v_response', v_response)
            value = float(v_response)
