...
def get_proposal(prompt, method='glm', temperature=0.7, max_tokens=2048, seed=170, max_length=2048, truncation=True,
                 do_sample=True, max_new_tokens=1024, client=None, served_model_name=None):
    response = []
    cnt = 2
    if method == 'glm':
        ...
        while not response and cnt:
            response = local_inference_model(prompt, max_length=max_length, truncation=truncation, do_sample=do_sample,
                                             max_new_tokens=max_new_tokens, temperature=temperature, client=client, served_model_name=served_model_name)
            cnt -= 1
        if not response:
            print(f'obtain<{method}>response fail!\n')
            return []
...
def get_value(prompt_answer, method='glm', temperature=0.7, max_tokens=1000, seed=170, max_length=2048, low=0, high=1, VALUE_BASE_MODEL_DIR='', client=None, served_model_name=None):
    response = []
    cnt = 2
    if method == 'glm':
        while not response and cnt:
        ...
        while cnt:
            try:
                value = local_value_model(prompt_answer, max_length=max_length, low=low, high=high, VALUE_BASE_MODEL_DIR=VALUE_BASE_MODEL_DIR, client=client, served_model_name=served_model_name)
                break
